...
def api_magia_madre():
    ...
    # 🔥 OBTENER LOS ESTAMPADOS RAW PARA RESPETAR EL ORDEN VISUAL Y ESPACIOS VACIOS 🔥
    ...
    try:
        ...
        # 🔥 DIVISIÓN MATEMÁTICA ESTRICTA (RESPETANDO LOS ESPACIOS VACÍOS DE LA APP) 🔥
        est_por_folio_raw = [raw_estampados[i:i + estampados_por_folio] for i in range(0, len(raw_estampados), estampados_por_folio)]
        
        # Ahora eliminamos los vacíos pero mantenemos las agrupaciones visuales por página
        est_por_folio = []
        estampados_limpios = []
        for chunk in est_por_folio_raw:
            clean_chunk = [e for e in chunk if e.strip()]
            est_por_folio.append(clean_chunk)
            estampados_limpios.extend(clean_chunk)
            
        if not estampados_limpios:
            estampados_limpios = ["SIN ESTAMPADO"]
        ...
        # 🔥 CERRAR LA BASE DE DATOS AQUÍ ANTES DE CREAR EL PDF 🔥
        if cursor: 
            cursor.close()
        if db: 
            db.close()
    ...
    try:
        ...
        for lote_idx, lote_estampados in enumerate(est_por_folio):
            for chunk_idx, color_chunk in enumerate(color_chunks):
                ...
                # Construir el Layout Final de la Hoja
